...
- **✅ 单元测试项目 `tests/StarMark.Tests`（xUnit, 11 用例全通过）** — `DittoSourceTests`（合成 DittoDB：文本/CF_HDROP 文件/分组跳过/实时检索/缺失库）、`EverythingCsvParseTests`（字段顺序 name,path,size,date；引号含逗号字段；size 标志开关；日期→Unix 秒；空行→null）
