...
| 🕒 时钟 | 日期与秒级时钟（仅可见时计时） |
| ✅ 待办 | 增、勾选完成、删除；**全部/未完成/已完成**分段筛选带计数、**优先级颜色标记**、**截止日期**、删除后**行内撤销条**、**拖拽排序**（顺序写入 `extra_json.order`），未完成在前 |
...
| 🌤️ 天气 | **Phase C**：实况（温度/体感/湿度/风力）+ 未来三天预报；支持 **°C/°F 切换**（华氏下风速同步转 mph）与 **「今日逐时」视图**；组件放大后自动追加 **降水概率 / 紫外线 / 气压 / 日出日落**（按尺寸三档分级，带滞回避免拖边缘时跳档）。数据源 Open-Meteo，**免费且无需 API Key**；WMO 天气码的中文/图标映射照搬 DeskBox 的 `WeatherCodeMapper`。点标题栏城市名可换城市（支持中文搜索），选择写入 `settings.json` |
| 🎵 音乐 | **Phase C**：曲目 / 艺人·专辑 / 来源应用 + 进度条 + 上一首·播放暂停·下一首。走 Windows 系统媒体传输控制（**SMTC**），因此**不需要对接任何具体播放器**——Spotify、网易云、浏览器里正在播的视频都能读和控制。同时开多个播放器时出现「切换音源」菜单：**跟随系统**或指定某个播放器（互斥，选定后不再被“最后播放”抢走）。**进度条可拖动跳转**（seek），并提供 **随机 / 列表循环** 模式切换 |
...
- **重命名**：组件右键「重命名…」或选中组件按 **F2**（也可双击标题栏）给单个组件起名字，写回 `widgets.json` 持久化；标题栏与胶囊标题优先显示自定义名，留空即恢复组件类型默认名。
- **键盘操作**：选中标题栏后 **Esc** 收起为胶囊、**Enter/Space** 把胶囊展开回原位、**F2** 重命名。
- **协同移动**：按住标题栏拖动时同时按 **Ctrl**，同一显示器上的所有组件一起同步位移——整屏布局整体挪位置，不必一个一个搬。
- **数据实时同步**：数据层每次写入都会广播一次变更，所有组件去抖重载。在主界面置顶/取消置顶、删除、改标签，或另一个组件里增删待办，桌面组件立刻跟着变（不会出现主界面改了、组件还是老的）。
