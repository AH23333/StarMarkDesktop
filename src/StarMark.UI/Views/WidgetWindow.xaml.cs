...
public sealed partial class WidgetWindow : Window
{
    ...
    private StackPanel? _addLinkForm;
    ...
    public WidgetWindow(WidgetKind kind, WidgetStorage storage, IItemRepository? repo, WidgetManager manager)
    {
        ...
        if (kind == WidgetKind.QuickLaunch)
    ...
    internal UIElement BuildQuickLaunch()
    {
        ...
        // 置顶条目（来自 StarMark 数据库 Pinned=1）
        var pinnedHost = new StackPanel();
        panel.Children.Add(SectionHeader("置顶条目", "🔄", (_, _) => _ = LoadPinnedAsync(pinnedHost)));
        panel.Children.Add(pinnedHost);

        panel.Children.Add(new Border
        {
            Height = 1,
            Margin = new Thickness(0, 4, 0, 4),
            Background = WidgetBrush("WidgetDividerBrush"),
        });

        // 用户自定义快捷入口
        panel.Children.Add(SectionHeader("快捷入口", "＋", (_, _) => ToggleAddLinkForm()));
        var linksHost = new StackPanel();
        panel.Children.Add(linksHost);

        _addLinkForm = AddLinkForm();
        _addLinkForm.Visibility = Visibility.Collapsed;
        panel.Children.Add(_addLinkForm);

        panel.Children.Add(new TextBlock
        {
            Text = "提示：可把文件、文件夹或网址直接拖入本组件",
            FontSize = 10,
            Opacity = 0.6,
        ...
        _ = LoadPinnedAsync(pinnedHost);
        ReloadLinks(linksHost);
        return panel;
    }
    ...
    private async void QuickLaunch_Drop(object sender, DragEventArgs e)
    {
        ...
        try
        {
            ...
            else if (v.Contains(StandardDataFormats.Text))
            {
                ...
                if (TryParseUri(text, out var uri) && uri is not null
    ...
    private void OnLinksChanged() => DispatcherQueue?.TryEnqueue(RebuildQuickLaunch);
    ...
    private StackPanel SectionHeader(string title, string actionGlyph, RoutedEventHandler action)
    {
    ...
    private async Task LoadPinnedAsync(StackPanel host)
    {
        host.Children.Clear();
        ...
        try
        {
            ...
            if (items.Count == 0)
            {
                host.Children.Add(EmptyHint("在条目卡片右键 → 置顶，即在此显示"));
                return;
            }
            foreach (var item in items)
                host.Children.Add(LinkRow(item.Title, item.Uri, item.Type switch
                {
                    ItemType.File => "📁",
                    ItemType.Bookmark => "🔖",
                    ItemType.GitHubStar => "⭐",
        ...
        {
            StarLog.Error("加载置顶条目失败", ex);
        }
    }

    private void ReloadLinks(StackPanel linksHost)
    {
        linksHost.Children.Clear();
        var data = _storage.Load();
        if (data.Links.Count == 0)
        {
            linksHost.Children.Add(EmptyHint("点 ＋ 添加常用文件、网址或程序"));
            return;
        }
        foreach (var link in data.Links)
            linksHost.Children.Add(LinkRow(link.Title, link.Uri, "🔗", removable: true));
    }

    private Grid LinkRow(string title, string uri, string emoji, bool removable)
    {
        ...
        ToolTipService.SetToolTip(btn, uri);
        var sp = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 6 };
        sp.Children.Add(new TextBlock { Text = emoji, FontSize = 11, VerticalAlignment = VerticalAlignment.Center });
        sp.Children.Add(new TextBlock
        {
            Text = string.IsNullOrWhiteSpace(title) ? uri : title,
            FontSize = 11,
        ...
        btn.Click += async (_, _) =>
        {
            ...
            catch (Exception ex) { StarLog.Error($"打开快捷入口失败: {uri}", ex); }
        };
        ...
        if (removable)
        {
            ...
            ToolTipService.SetToolTip(del, "移除");
            del.Click += (_, _) => _ = RemoveLinkAsync(uri);
    ...
    private async Task RemoveLinkAsync(string uri)
    {
        await _manager.RemoveLinkAsync(uri);
        RebuildQuickLaunch();
    }

    private StackPanel AddLinkForm()
    {
        ...
        var nameBox = new TextBox { PlaceholderText = "名称（可留空）", FontSize = 11, Padding = new Thickness(6, 4, 6, 4) };
        var uriBox = new TextBox { PlaceholderText = "网址 / 文件路径（粘贴后回车）", FontSize = 11, Padding = new Thickness(6, 4, 6, 4) };
        var addBtn = new Button { Content = "添加", HorizontalAlignment = HorizontalAlignment.Right, Padding = new Thickness(12, 3, 12, 3) };

        async Task Submit()
        {
            var raw = uriBox.Text.Trim();
            if (!TryParseUri(raw, out var parsed) || parsed is null)
            {
                uriBox.Focus(FocusState.Programmatic);
                return;
            }
            var name = nameBox.Text.Trim();
            if (string.IsNullOrEmpty(name))
                name = parsed.IsFile ? System.IO.Path.GetFileName(parsed.LocalPath) : parsed.Host;
            await _manager.AddLinkAsync(name, parsed.AbsoluteUri);
            nameBox.Text = uriBox.Text = string.Empty;
            form.Visibility = Visibility.Collapsed;
            RebuildQuickLaunch();
        }

        addBtn.Click += async (_, _) => await Submit();
        uriBox.KeyDown += (_, k) => { if (k.Key == VirtualKey.Enter) _ = Submit(); };

        form.Children.Add(nameBox);
        form.Children.Add(uriBox);
    ...
    {
        if (_addLinkForm is null) return;
        _addLinkForm.Visibility = _addLinkForm.Visibility == Visibility.Visible
            ? Visibility.Collapsed : Visibility.Visible;
        if (_addLinkForm.Visibility == Visibility.Visible && _addLinkForm.Children[1] is TextBox box)
